...
(() => {
  ...
  // === Serializador universal do form ===
  function serializeFormToPayload(formEl) {
    ...
    // data-k (quando inputs no preview)
  ...
  async function postJSON(url, body, extraHeaders = {}) {
    ...
    const resp = await fetch(url, {
      method: 'POST', mode: 'cors', credentials: 'omit', headers, body: JSON.stringify(body)
    });
    if (!resp.ok) {
  ...
  async function fetchJSON(url, opts = {}, meta = {}) {
    ...
    if (method === 'GET') return await getJSON(url, headers);
    const resp = await fetch(url, {
  ...
  const el = {
    ...
    ugNome:  $('#ug_nome'), ugCnpj:  $('#ug_cnpj'), ugEmail: $('#ug_email'),
    ugOrgaoVinc: $('#ug_orgao_vinc'),
  ...
  function obterCNPJUG() {
    const candidatos = [
      ...
      el.cnpjUg?.value, el.ugCnpj?.value
    ].filter(Boolean);
    const limpo = candidatos.map(v => String(v).replace(/\D/g, '')).find(v => v && v.length === 14);
    return limpo || null;
  }
  function bindMasks(){
  ...
  function migrateFromAdesaoIfNeeded() {
    try {
      ...
      const next = { step: 1, values: mapped, lastSaved: Date.now(), finalizedAt: 0 };
  ...
  function ensureBackButton(modalId){
    ...
    if (!el) return;
    const content = el.querySelector('.modal-content'); if (!content) return;
    let footer = content.querySelector('.modal-footer');
    if (!footer) { footer = document.createElement('div'); footer.className = 'modal-footer'; content.appendChild(footer); }
  ...
  async function onPesquisar(ev){
    ...
    try {
      ...
      if (ok && !isGesconNumber(nGescon)) { console.warn('Número Gescon inválido:', nGescon); ok = false; }
      ...
      if (err && err.status === 404) {
        ...
        el.boxGescon && el.boxGescon.classList.add('d-none');
        if (el.infoDataEncGescon) el.infoDataEncGescon.textContent = '—';
        try { await hidratarTermosRegistrados(cnpj); } catch {}
        showModal('modalGesconNaoEncontrado');
      } else {
  ...
  async function hidratarTermosRegistrados(cnpj){
    ...
    try{
      ...
      // 3) CRP anterior
      const dataVenc =
        crp.data_venc || crp.DATA_VALIDADE_DMY || crp.DATA_VALIDADE_ISO ||
        crp.DATA_VALIDADE || crp.data_validade || crp.validade || crp.vencimento || '';
      ...
      } catch (e) { console.warn('Espelho __TERMO_DATA__ falhou:', e); }
      ...
      if (el.selectTipoUltCrp) el.selectTipoUltCrp.value = tipo || '';
      else { if (el.tipoAdm) el.tipoAdm.checked = (tipo === 'Administrativa'); if (el.tipoJud) el.tipoJud.checked = (tipo === 'Judicial'); }

      if (window.__DEBUG_SOLIC_CRP__) {
        try {
          const _venc = (el.dataUltCrp?.value ?? '').trim() || null;
          const _tipo = (el.selectTipoUltCrp?.value ?? '').trim()
            || ((el.tipoAdm?.checked ? 'Administrativa' : (el.tipoJud?.checked ? 'Judicial' : '')) || '').trim() || null;
          console.log('[E1] hidratarTermosRegistrados → CRP', { '3.1_DATA_VENC_ULTIMO_CRP': _venc, '3.2_TIPO_EMISSAO_ULTIMO_CRP': _tipo });
        } catch {}
      }
      ...
      if (!irregs.length) {
        ...
        if (s) { irregs = String(s).split(/;|,/).map(t => t.trim()).filter(Boolean); }
      }
  ...
  function validarFaseSelecionada(){
    ...
    if (f==='4.3'){
      ...
      if(!ok){ return { ok:false, motivo:'Na fase 4.3, marque ao menos um critério ou descreva/justifique no(s) campo(s).' }; }
    }
    if (f==='4.4'){
      const optE = document.getElementById('F442_OPTE');
      if (optE?.checked) {
        const crits = $$('#F44_CRITERIOS input[type="checkbox"]:checked');
        if (!crits.length) { return { ok:false, motivo:'Na 4.4, ao marcar a finalidade “e)”, selecione pelo menos um critério em 4.4.3.' }; }
      }
    }
    if (f==='4.5'){
  ...
  function popularListasFaseComBaseNosCritérios(){
    ...
    // Reflete seleções em __TERMO_DATA__
    try {
      window.__TERMO_DATA__ = window.__TERMO_DATA__ || {};
      ...
      console.warn('Erro ao sincronizar __TERMO_DATA__', e);
    }
  }
  /* ========= Validação geral ========= */
  ...
  function buildPayload(){
    ...
    const F41_OPCAO_CODE = (()=>{
      ...
      return m ? m[0] : (document.querySelector('input[name="F41_OPCAO"]:checked')?.value || '').trim();
    })();
    ...
    const _dataVencUltCrpRaw =
      byNameVal('DATA_VENC_ULTIMO_CRP') || byNameVal('DATA_VENCIMENTO_ULTIMO_CRP') ||
      byIdVal('DATA_VENC_ULTIMO_CRP')  || byIdVal('DATA_VENCIMENTO_ULTIMO_CRP') ||
      byNameVal('data_venc_ult_crp')   || byIdVal('data_venc_ult_crp') || '';
    ...
    const _tipoEmissaoUltCrpRaw =
      byNameVal('TIPO_EMISSAO_ULTIMO_CRP') || byIdVal('TIPO_EMISSAO_ULTIMO_CRP') ||
      byNameVal('tipo_emissao_ult_crp')    || byIdVal('tipo_emissao_ult_crp') ||
    ...
    obj.CRP = Object.assign({}, obj.CRP || {}, { data_venc: obj.DATA_VENC_ULTIMO_CRP, tipo: obj.TIPO_EMISSAO_ULTIMO_CRP });
    ...
    ['F42_LISTA','F43_LISTA','F44_CRITERIOS','F44_DECLS','F44_FINALIDADES','F46_CRITERIOS','F46_FINALIDADES','F462F_CRITERIOS','CRITERIOS_IRREGULARES']
      .forEach(k => { const v = obj[k]; const arr = Array.isArray(v) ? v : (v ? [String(v)] : []); obj[k+'[]'] = arr; });

    ['F43_INCLUIR','F43_INCLUIR_B'].forEach(k => {
      const s = (obj[k] ?? '').toString(); obj[k] = s; obj[k+'[]'] = s ? s.split(';').map(t => t.trim()).filter(Boolean) : [];
    });
    ...
    try {
      ...
      if (form) {
        ...
        for (const [k, v] of Object.entries(extra)) {
          const curr = obj[k];
          const isEmpty = curr == null || (typeof curr === 'string' && curr.trim() === '') || (Array.isArray(curr) && curr.length === 0);
          if (isEmpty) obj[k] = v;
  ...
  function makeSolicCrpCompatFields(p) {
    ...
    if (!DEFICIT_ATUARIAL && (p.F43_PLANO || p.F43_DESC_PLANOS)) DEFICIT_ATUARIAL = '4.3';
  ...
  async function gerarBaixarPDF(payload){
    ...
    try { await fetchJSON(api('/warmup'), {}, { label: 'warmup', timeout: 8000, retries: 1 }); } catch {}
  ...
  el.btnGerar?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (window.__DEBUG_SOLIC_CRP__) {
        try { console.log('[solic_crp] buildPayload() (PDF) →', JSON.stringify(payload, null, 2)); } catch {}
      }
      console.log('DEBUG payload (pre-send):', {
        F44_CRITERIOS: payload.F44_CRITERIOS, F44_DECLS: payload.F44_DECLS,
  ...
  form?.addEventListener('submit', async (ev)=>{
    ...
    try {
      ...
      if (window.__DEBUG_SOLIC_CRP__) { try { console.log('[SUBMIT] resposta API →', resp); } catch {} }
  ...
  function initWelcome(){ const mw = $('#modalWelcome'); if(mw){ setTimeout(()=> bootstrap.Modal.getOrCreateInstance(mw).show(), 150); } }

  function showErro(msgs) {
    try {
      const ul = document.getElementById('modalErroLista');
      if (ul) {
        ul.innerHTML = (Array.isArray(msgs) ? msgs : [String(msgs||'Ocorreu um erro.')]).map(m => `<li>${m}</li>`).join('');
      }
  ...
  function initEtapa1Bridges() { wireSituacaoEspelhamento(); if (window.__REGISTRO_TERMO__) { preencherRegistrosDoTermo(window.__REGISTRO_TERMO__); } }
  ...
  // Watcher F4.3 + justifs
  (function(){
    const KEYS = ['f43_plano','f43_just','justificativas_gerais'];
    function getVal(k){ return (window.__TERMO_DATA__?.[k] ?? '').toString().trim() || 'Não informado'; }
    function setVal(el, v){
      if (el.matches('input, textarea, select')) { el.value = v; el.dispatchEvent(new Event('input',  { bubbles:true })); el.dispatchEvent(new Event('change', { bubbles:true })); }
